...
template <int dim, int spacedim>
bool
cell_has_artificial_child(
const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
{
int number_ranks;
MPI_Comm_size(MPI_COMM_WORLD, &number_ranks);
if (number_ranks==1) return false;
Assert(cell->is_active() == false, ExcInternalError());
bool artificial_child = false;
for (unsigned int i = 0; i < cell->n_children(); ++i)
if (cell->child(i)->is_active() && cell->child(i)->is_artificial())
artificial_child=true;
return (artificial_child);
}

template <int dim, int spacedim>
bool
cell_contains_artificial(
const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
{
int number_ranks;
MPI_Comm_size(MPI_COMM_WORLD, &number_ranks);
if (number_ranks==1) return false;
bool artificial_presence=false;
if (cell->is_active())
artificial_presence = cell->is_artificial();
else for (unsigned int i = 0; i < cell->n_children(); ++i)
artificial_presence |= cell_contains_artificial(cell->child(i));
return (artificial_presence);
}

template <int dim, int spacedim>
bool
cell_is_fully_locally_owned(
const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
{
int number_ranks;
MPI_Comm_size(MPI_COMM_WORLD, &number_ranks);
...
bool fully_locally_owned=true;
if (cell->is_active())
fully_locally_owned = cell->is_locally_owned();
else for (unsigned int i = 0; i < cell->n_children(); ++i)
fully_locally_owned &= cell_is_fully_locally_owned(cell->child(i));
return (fully_locally_owned);
}

template <int dim, int spacedim>
bool
cell_contains_locally_owned(
const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
{
int number_ranks;
MPI_Comm_size(MPI_COMM_WORLD, &number_ranks);
if (number_ranks==1) return true;
bool contains_locally_owned=false;
if (cell->is_active())
contains_locally_owned = cell->is_locally_owned();
else for (unsigned int i = 0; i < cell->n_children(); ++i)
contains_locally_owned |= cell_contains_locally_owned(cell->child(i));
return (contains_locally_owned);
}

template <int dim, int spacedim>
bool
belongs_to_cell(const TriaIterator<dealii::CellAccessor<dim, spacedim>> &child,
const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
{
bool belongs=false;
for (unsigned int i = 0; i < cell->n_children(); ++i)
{
if (cell->child(i)==child) return true;
else belongs |= belongs_to_cell(child, cell->child(i));
}
return (belongs);
}
...
for (const auto &cell : cell_iterators()) {
if (cell->is_active()//&&cell_has_artificial_child(cell)
) continue;
if (!cell_is_fully_locally_owned(cell)) {
if (!cell_contains_locally_owned(cell)) continue;
for (unsigned int i = 0; i < cell->n_children(); ++i) {
if (!cell->child(i)->is_active()) continue;
if (cell->child(i)->is_locally_owned()
&& !cell->child(i)->refine_flag_set()) {
for (unsigned int k = 0; k < cell->child(i)->n_faces(); ++k) {
if (cell->child(i)->at_boundary(k)) continue;
if (cell->child(i)->neighbor(k)->is_active()
//&& cell->child(i)->child(j)->neighbor(k)->is_ghost()
&& belongs_to_cell(cell->child(i)->neighbor(k), cell)) {
if (cell->child(i)->neighbor(k)->refine_flag_set())
cell->child(i)->set_refine_flag();
}
}
}
}
...
for (unsigned int i = 0; i < cell->n_children(); ++i) {
if (!cell->child(i)->is_active()) {
for (unsigned int j = 0; j < cell->child(i)->n_children(); ++j) {
if (cell->child(i)->child(j)->is_active()
&& cell->child(i)->child(j)->is_locally_owned()
&& cell->child(i)->child(j)->coarsen_flag_set()) {
for (unsigned int k = 0; k < cell->child(i)->child(j)->n_faces(); ++k) {
if (cell->child(i)->child(j)->at_boundary(k)) continue;
if (cell->child(i)->child(j)->neighbor(k)->is_active()
//&& cell->child(i)->child(j)->neighbor(k)->is_ghost()
&& belongs_to_cell(cell->child(i)->child(j)->neighbor(k),cell))
{
if (!cell->child(i)->child(j)->neighbor(k)->coarsen_flag_set())
cell->child(i)->child(j)->clear_coarsen_flag();
}
}
}
}
...
else {//standard case of locally owned patch
Assert(cell_is_patch_level_1(cell), ExcInternalError());

if (cell->child(0)->is_active())
continue;
...
for (unsigned int i = 0; i < n_children; ++i) {
Assert(cell_is_patch_level_1(cell->child(i)), ExcInternalError());
...
Assert(cell_is_patch_level_1(child), ExcInternalError());
