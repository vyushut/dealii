...
//{A 2D test about the flux sparsity pattern for an anisotropic mesh with hanging nodes same as in
// sparsity/face_sparsity_pattern_2d_anisotropic.cc. Here, we assemble and output the flux matrix without
...
{
   deallog.get_file_stream() << "Visiting the face " << face_index << " on cell " << cell->index() << std::endl;
   return true;
}
...
template <int dim>
void
create_and_output_flux_pattern(DoFHandler<dim> &dof_handler, DynamicSparsityPattern &dsp, AffineConstraints<double>& constraints)
{
  deallog << "Exactly 4 faces should be visited for creating the flux sparsity pattern + 4 faces visited during the assembly" << std::endl;
  ...
  DoFTools::make_flux_sparsity_pattern(dof_handler,
                                       ...
                                       [&](const auto &       cell,
                                           const unsigned int face_index) {
      return just_visiting<dim>(cell, face_index);
                                       });
}


template <int dim>
void assembly_and_print(const FiniteElement<dim>& fe, DoFHandler<dim> &dof_handler, SparsityPattern &sparsity_pattern,
              AffineConstraints<double>& constraints, SparseMatrix<double> &global_matrix
) {
    ...
    for (const auto &cell:
        ...
        for (unsigned int f: cell->face_indices()) {
            ...
            if (!cell->neighbor_is_coarser(f)
                ...
                continue;
            if (!just_visiting<dim>(cell, f)) continue;
            ...
            for (unsigned int q = 0;
                ...
                for (unsigned int i = 0; i < n_interface_dofs; ++i)
                    for (unsigned int j = 0; j < n_interface_dofs; ++j) {
                        local_stabilization(i, j) += (24) * normal *
            ...
            constraints.distribute_local_to_global(local_stabilization, local_interface_dof_indices, global_matrix);
        }
    }
}
...
check()
{
    ...
    assembly_and_print<dim>(finite_element, dof_handler, sparsity_pattern, constraints, global_matrix);
